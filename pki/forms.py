...
class CertificateAuthorityForm(forms.ModelForm):
    ...
    passphrase_verify = forms.CharField(widget=forms.PasswordInput,
                                        required=False)
    parent_passphrase = forms.CharField(widget=forms.PasswordInput,
                                        required=False)
...
class CertificateForm(forms.ModelForm):
    ...
    parent_passphrase = forms.CharField(widget=forms.PasswordInput,
                                        required=False)
    pkcs12_passphrase = forms.CharField(widget=forms.PasswordInput,
                                        required=False)
    pkcs12_passphrase_verify = forms.CharField(widget=forms.PasswordInput,
                                               required=False)
...
class x509ExtensionForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if bc in ('root_ca', 'edge_ca'):
            if len(eku) > 0:
                self._errors['extended_key_usage'] = ErrorList(
                    ['You cannot set extendedKeyUsage for a CA extension'])
            if eku_c is True:
                self._errors['extended_key_usage_critical'] = ErrorList(
                    ['You cannot set extendedKeyUsage to critical for a CA \
                     extension'])
        elif bc == 'enduser_cert':
            if len(eku) < 1:
                self._errors['extended_key_usage'] = ErrorList(
                    ['extendedKeyUsage is required for a non-CA extension'])
