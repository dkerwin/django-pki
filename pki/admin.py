...
l_hdlr.setFormatter(logging.Formatter("%(asctime)s %(levelname)s - %(module)s.\
...
class Certificate_Authority_Admin(CertificateBaseAdmin):
    ...
    list_display = ('id', 'common_name', 'Serial_align_right', 'Valid_center',
                    'Chain_link', 'Tree_link', 'Parent_link', 'Expiry_date',
    ...
    readonly_fields = ('Expiry_date', 'Creation_date', 'Revocation_date',
                       'serial', 'Chain', 'Certificate_Dump', 'CA_Clock',
                       'State',)
    fieldsets = (('Define action', {'fields': ('action',), },),
                 ...
                 ('Certificate', {'fields': ('State', 'common_name', 'name',
                                             'country', 'state', 'locality',
                                             'organization', 'OU', 'email',
                                             'key_length', 'valid_days',
                                             'extension', 'passphrase',
                                             'passphrase_verify', 'serial',
                                             'Expiry_date', 'Creation_date',
                                             'Revocation_date',),
                                  'classes': ['wide', ], },),
                 ('Encoding options', {'fields': ('der_encoded',), },),
                 ('Certificate signing', {'fields': ('CA_Clock', 'Chain',
                                                     'parent',
                                                     'parent_passphrase',
                                                     'crl_dpoints', 'policy',),
                                          'classes': ['wide', ], },),
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name == "parent":
            ...
            kwargs["queryset"] = x509Extension.objects.filter(\
                                    basic_constraints__contains="CA:TRUE", \
                                    key_usage__name__contains="keyCertSign")
            return db_field.formfield(**kwargs)
        return super(Certificate_Authority_Admin, self).\
...
class Certificate_Admin(CertificateBaseAdmin):
    ...
    list_display = ('id', 'common_name', 'Serial_align_right', 'Valid_center',\
                    'Chain_link', 'Parent_link', 'Expiry_date', 'Description',\
                    'Creation_date', 'Revocation_date', 'Download_link', \
                    'Email_link')
    list_display_links = ('common_name',)
    radio_fields = {"action": admin.VERTICAL}
    list_filter = ('parent', 'active', 'extension',)
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name == "parent":
            ...
            f = Q(basic_constraints__contains="CA:FALSE") | ((
                Q(basic_constraints__contains="CA:TRUE") &
                Q(basic_constraints__contains="pathlen:0")) &
                ~Q(key_usage__name__contains="keyCertSign"))
            
            kwargs["queryset"] = x509Extension.objects.filter(f)
            return db_field.formfield(**kwargs)
        
        return super(Certificate_Admin, self).formfield_for_foreignkey( \
...
class x509Extension_Admin(CertificateBaseAdmin):
    ...
    list_display = ('id', 'name', 'description', 'basic_constraints', \
                    'key_usage_csv', 'ext_key_usage_csv', 'created', \
                    'CrlDpoint_center',)
    list_display_links = ('name',)
    search_fields = ['name', 'description', ]
    date_hierarchy = 'created'
    fieldsets = (('X509 extension', {'fields': ('name', 'description', \
                                                'basic_constraints', \
    ...
    def delete_view(self, request, object_id, extra_context=None):
        ...
        if x509.certificateauthority_set.all() or x509.certificate_set.all():
            logger.error("x509 extension \"%s\" cannot be removed because \
                         it is in use!" % x509.name)
            messages.error(request, 'x509 extension "%s" cannot be removed \
                           because it is in use!' % x509.name)
            return HttpResponseRedirect("../../")
        else:
            return super(x509Extension_Admin, self).delete_view(request, \
