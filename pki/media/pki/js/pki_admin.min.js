$(document).ready(function(){UpdateTime();setInterval(UpdateTime,1E3);if($("input[id=id_pkcs12_encoded]")){$("#id_pkcs12_passphrase").bind("change keyup focus",{a:"#id_pkcs12_passphrase",b:"#id_pkcs12_passphrase_verify"},onPfChange);$("#id_pkcs12_passphrase_verify").bind("change keyup focus",{a:"#id_pkcs12_passphrase",b:"#id_pkcs12_passphrase_verify"},onPfChange)}if(window.__pki_add__!="True"){onActionChange();$("input[name=action]").change(onActionChange);$("form").submit(function(){!$("input[id=id_action_3]").attr("checked")&&
window.__pki_model__=="CertificateAuthority"&&$("input[id=id_passphrase]").val("XXXXXXXXXXXXXXXXXXXXXXXXXX");$("#certificateauthority_form :input:not(:submit), #certificate_form :input:not(:submit)").each(function(a,b){$(b).removeAttr("disabled")})})}else{$("input[id=id_action_1]").attr("disabled","disabled");$("input[id=id_action_2]").attr("disabled","disabled");$("input[id=id_action_3]").attr("disabled","disabled");$("label[for=id_passphrase_verify]").addClass($("label[for=id_passphrase]").attr("class"));
onParentChange();onCnChange();$("#id_parent").change(onParentChange);$("#id_common_name").bind("change keyup",onCnChange);$("#id_passphrase").bind("change keyup focus",{a:"#id_passphrase",b:"#id_passphrase_verify"},onPfChange);$("#id_passphrase_verify").bind("change keyup focus",{a:"#id_passphrase",b:"#id_passphrase_verify"},onPfChange)}});
